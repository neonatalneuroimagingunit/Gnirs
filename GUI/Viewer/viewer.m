...
function channel_table_selection_callback(handle, evnt, GHandle, vIdx)
channelname = handle.Data(evnt.Indices(:,1),1);
[~, idx, ~] = intersect(GHandle.Viewer(vIdx).Probe.channel.label, channelname);
temp = GHandle.Viewer(vIdx).Probe.channel.pairs(idx,:);
tobesearched = cellstr(num2str(temp,'s%.3d_d%.3d'));
GHandle.Viewer(vIdx).WatchList.edvLine = contains({GHandle.Viewer(vIdx).timeplot.lines1.Tag}, tobesearched);
