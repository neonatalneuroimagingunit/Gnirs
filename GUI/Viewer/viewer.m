...
GHandle.Viewer.WatchList.colorLine = sorting_colors(nchannels, sortingmethod);
...
for ii = 1:1:nchannels
    GHandle.Viewer.timeplot.lines1(ii) = plot(GHandle.CurrentDataSet.Data.reltime, nirsdata(:,ii),...
							'LineWidth', 1, 'LineStyle', '-',...
							'buttonDownFcn', {@line_callback , GHandle},...
							'Color', GHandle.Viewer.WatchList.colorLine(ii,:),...
...
GHandle.Viewer.listener = addlistener(GHandle.Viewer.WatchList,'edvLine','PostSet',@(src,evnt)change_width(src,evnt,GHandle));
...
for ii = 1:1:size(nirsdata,2)
    ...
    GHandle.Viewer.timeplot.lines2(ii) = plot(GHandle.CurrentDataSet.Data.reltime, nirsdata(:,ii),...
						'LineWidth', 1, 'LineStyle', '-',...
						'Color', GHandle.Viewer.WatchList.colorLine(ii,:),...
						'buttonDownFcn', {@move_time_window, GHandle});
...
GHandle.Viewer.WatchList.edvLine = ones(size(GHandle.Viewer.timeplot.lines2));
...
for ii = 1:1:nchannels
    ...
    GHandle.Viewer.spectrumplot.lines1(ii).Color = GHandle.Viewer.WatchList.colorLine(ii,:);
...
set(GHandle.Viewer.spectrumplot.lines1_selection, 'buttonDownFcn', {@line_callback, GHandle})
...
for ii = 1:1:size(nirsdata,2)
    ...
    set(GHandle.Viewer.spectrumplot.lines2(ii), 'Color', GHandle.Viewer.WatchList.colorLine(ii,:));
    aa = aa+1;
end
