...
%     'Units', 'normalize', ...
%     'ColumnName', { 'S', 'D','Wavelength', 'SD distance (mm)','Name'}, ...
...
GHandle.Viewer(vIdx).channelPanel.channeltable = uiw.widget.Table(...
    'Parent', GHandle.Viewer(vIdx).channelPanel.channel, ...
    'Units', 'normalize', ...
    'ColumnName', { 'S', 'D','Wavelength', 'SD distance (mm)','Name'}, ...
    'ColumnEditable',[false false false false], ...
    ...
    'LabelLocation','top',...
    'LabelHeight',18);
GHandle.Viewer(vIdx).channelPanel.eventtable = uiw.widget.Table(...
    'Parent',  GHandle.Viewer(vIdx).channelPanel.event, ...
    'Units', 'normalize', ...
    'ColumnName', {'Label', 'Code', 'Start (s)', 'Duration (s)'}, ...
    'ColumnEditable',[false false false false], ...
...
GHandle.Viewer(vIdx).timeplot.panel = uipanel('parent', GHandle.Viewer(vIdx).tabs.time,...
    ...
    'ShadowColor', [211 211 211]/255, ...
    'BorderType', 'none', ...
    'FontWeight', 'bold', ...GHandle.Viewer(vIdx).Preference
    'FontSize', 8, ...
    'FontName', defaultFontName, ...
    'Visible', 'on');
...
GHandle.Viewer(vIdx).spectrumplot.panel = uipanel('parent', GHandle.Viewer(vIdx).tabs.frequency,...
    ...
    'BackgroundColor', backgroundColor, ...
    'ForegroundColor', foregroundColor, ...
    'HighlightColor', highlightColor, ...handle
    'ShadowColor', [211 211 211]/255, ...
    'BorderType', 'none', ...
    'FontWeight', 'bold', ...
    'FontSize', 8, ...
...
if contains(tobesearched,'Ch')
    temp = GHandle.Viewer(vIdx).Probe.channel.pairs(strcmp(GHandle.Viewer(vIdx).Probe.channel.label, tobesearched),:);
...
function channel_table_selection_callback(~, evnt, GHandle, vIdx)

idx = evnt.SelectedRows;
if ~isempty(idx)
    tempMask = false(size(GHandle.Viewer.WatchList.edvLine));
    tempMask(idx) = true;
    GHandle.Viewer(vIdx).WatchList.edvLine = tempMask;
end
end

function event_table_selection_callback(handle, evnt, GHandle, vIdx)
end
