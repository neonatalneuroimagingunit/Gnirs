...
if obj.PhotonCheckbox.Value == true
    ...
    if obj.DataType.Value == 1
        photonDensity = sum(abs([obj.Forward.srcFlux(:, obj.SrcList.Value) obj.Forward.detFlux(:, obj.DetList.Value)]), 2);
    else
        src = obj.Probe.channel.pairs(obj.ChannelList.Value,1);
        det = obj.Probe.channel.pairs(obj.ChannelList.Value,2);
        photonDensity = sum(abs(obj.Forward.srcFlux(:,src).*obj.Forward.detFlux(:,det)),2);
    end
    
    photonDensity = photonDensity ./ max(photonDensity);
    ...
    downsamplingFactor = max(str2double(obj.DownSampling.String), 1);
    ...
    sensitivityMask = downSampleMask;
